...
public:
	...
	void LoadModel(std::wstring_view fileName, std::wstring_view resourceName);
...
void ResourceManager::LoadModel(std::wstring_view fileName, std::wstring_view resourceName)
{
	// 중복 검사
	...
	// 파일 경로 설정 및 유효성 검사
	const fs::path filePath = modelPath / fs::path(fileName);
	if (!exists(filePath) or filePath.extension() != ".fbx")
	{
		MessageBoxW(nullptr, L"File does not exist", L"LOG_ERROR", MB_OK);
		return;
	}
...
void ResourceManager::GetModel(std::wstring_view resourceName, std::weak_ptr<GameObject> gameObject)
{
	...
	{
		const auto founded = models.find(resourceName.data());
		...
		GetStaticModelData(resourceName, founded->second, gameObject);
	}
}
